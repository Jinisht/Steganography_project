...
Status encode_byte_to_lsb(char data, char *image_buffer){
    //clear lsb of image buffer[1-8]1111 1110-mask
    for(int i=0; i<8; i++){
        image_buffer[i] = (image_buffer[i] & 0xFE) | ((data >>i) & 1); //get bit from lsb side of data   
    }
    //set a bit in lsb of image buffer
return e_success;
}
...
Status encode_secret_file_data(EncodeInfo *encInfo){ // to read ascii file--fscanf, fgetc
    char str[encInfo->size_secret_file];
    fseek(encInfo->fptr_secret,0, SEEK_SET);
    fgets(str, encInfo->size_secret_file, encInfo->fptr_secret); //fegts(str, int size, file stream)
    encode_data_to_image(str, encInfo->size_secret_file, encInfo);
    return e_success;
}

Status copy_remaining_img_data(EncodeInfo *encInfo){
    int remaining_byte_size = 54+encInfo->image_capacity - ftell(encInfo->fptr_src_image);
    char str[remaining_byte_size];
    // read from source file
    fread(str, remaining_byte_size, 1, encInfo->fptr_src_image);

    // write to dest file
    fwrite(str, remaining_byte_size, 1, encInfo->fptr_stego_image);
    return e_success;
}
...
Status do_encoding(EncodeInfo *encInfo){
    if (open_files(encInfo)==e_success){
        ...
        if (check_capacity(encInfo)==e_success){
            ...
            printf("Check capacity id Failure\n");
            return e_failure;
        }
    }
    else{
